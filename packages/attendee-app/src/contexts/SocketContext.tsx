...
export const SocketProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const joinSession = (
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.once(AttendeeEvents.JOINED, (response: AttendeeJoinedPayload) => {
        ...
        if (response.currentState?.session?.currentActivity) {
          setCurrentActivity(response.currentState.session.currentActivity);
        }
  ...
  const submitResponse = (activityId: string, answer: any): Promise<void> => {
    return new Promise((resolve, reject) => {
      if (!socket) {
        reject(new Error('Socket not initialized'));
        return;
